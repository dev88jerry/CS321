...
/**
 * A player strategy that selects the best 11 players
 */
public class BestXI implements PlayerStrategy{

    /**
     * Method that is required to be implemented
     * Sets the 4 player positions into different array list
     * Selects the best players based on their attributes into the Team startingXI
    ...
    public void pickXI(Team team) {
        ...
        team.startingXI.add(keepers.get(keepers.size()-1));
        for(int i = 0; i < 4; i++){
            team.startingXI.add(defenders.get(defenders.size() - i - 1));
            team.startingXI.add(midfielders.get(midfielders.size() - i - 1));
            if(i<2)
                team.startingXI.add(attackers.get(attackers.size() - i - 1));
        }
    }
}
