 ...
 * Name: Jerry Lau
 * Date: 28 Apr 2022
 * Description: Attacker class that extends Player
 */
...
/**
 * Specific type of player with high attacking attributes
 */
public class Attacker extends Player{

    private static final double stdDev = 1.5;
    private static final double meanSt = 14.0;

    /**
     * Attacker constructor and sets the position
     * Generates a random name for the object
     * @param pos char of position
     */
    public Attacker(char pos){
        super();
        super.position = pos;
        super.name = "ATK " + super.generateName();
    }

    /**
     * Method to generate attacker stats
     * Uses a random normal distribution with constants defined at top of class
     * Attacker has more attacking stats than defending stats
     */
    @Override
    void generateStats() {
        ...
        super.techDef = (int) Math.round(mySample) - 2;
        mySample = ran.nextGaussian() * stdDev + meanSt;
        super.menDef = (int) Math.round(mySample) - 2;
        mySample = ran.nextGaussian() * stdDev + meanSt;
        super.phyDef = (int) Math.round(mySample) - 2;
        mySample = ran.nextGaussian() * stdDev + meanSt;
